#import "index.typ": fcl-zulip-join-url, rheo-source-url, sidebar-site
#show: sidebar-site.with(current: "faq")
...
It allows you to configure how certain source files should be merged (to produce a 'combined' EPUB or PDF file, for example, via #link(<spines>)[spines]), and also allows you to enrich certain outputs (such as HTML via #link(<custom-js-css>)[custom CSS]) with non-Typst content.
...
We have built Rheo as infrastructure for #link("https://chi-star-workshop.github.io/src/assets/pdf/papers/DocumentInfrastructureForAugmentedReading%20-%20Will%20Crichton.pdf")[augmented reading research] in Typst -- see the #link(<core>)[Core] page for more on how the plugin architecture enables this.
